...
preferences {
        input("ip", "string", title:"IP Address", description: "192.168.3.25", defaultValue: "192.168.3.25" ,required: true, displayDuringSetup: true)
...
metadata {
	definition (name: "Copy Raspberry Pi", namespace: "nicholaswilde", author: "Nicholas Wilde") {
		capability "Polling"
		capability "Refresh"
	...
	tiles {
        ...
        standardTile("contact15", "device.contact15", width: 1, height: 1) {
        ...
        standardTile("contact17", "device.contact17", width: 1, height: 1) {
			state("closed", label:'${name}', icon:"st.contact.contact.closed", backgroundColor:"#79b821", action: "open")
			state("open", label:'${name}', icon:"st.contact.contact.open", backgroundColor:"#ffa81e", action: "close")
		}
        standardTile("contact18", "device.contact18", width: 1, height: 1) {
			state("closed", label:'${name}', icon:"st.contact.contact.closed", backgroundColor:"#79b821", action: "open")
			state("open", label:'${name}', icon:"st.contact.contact.open", backgroundColor:"#ffa81e", action: "close")
		}
        ...
        main "button"
        details(["button", "temperature", "cpuPercentage", "memory" , "diskUsage", "contact15", "contact17", "contact18", "restart", "refresh"])
    }
}
...
def parse(String description) {
    ...
    if (result.containsKey("disk_usage")) {
  	...
  	if (result.containsKey("gpio_value_15")) {
    	log.debug "gpio_value_15: ${result.gpio_value_15.toDouble().round()}"
        if (result.gpio_value_15.contains("0")){
        	log.debug "gpio_value_15: open"
            sendEvent(name: "contact15", value: "open")
        } else {
        	log.debug "gpio_value_15: closed"
            sendEvent(name: "contact15", value: "closed")
        }
    }
    if (result.containsKey("gpio_value_17")) {
        ...
        if (result.gpio_value_17.contains("0")){
            ...
            sendEvent(name: "contact17", value: "open")
        } else {
        	log.debug "gpio_value_17: closed"
            sendEvent(name: "contact17", value: "closed")
        }
    }
    if (result.containsKey("gpio_value_18")) {
        ...
        if (result.gpio_value_18.contains("0")){
            ...
            sendEvent(name: "contact18", value: "open")
        } else {
        	log.debug "gpio_value_18: closed"
            sendEvent(name: "contact18", value: "closed")
        }
    }
...
private delayAction(long time) {
	new physicalgraph.device.HubAction("delay 1000") //brent edit $time to 1000
}
